...
class Output(object):
    ...
    def write(self, message):
        with open("templog.txt", "w") as proc:
            proc.write(message)

    def is_present(self):
        cwd = os.getcwd()
        path = os.path.isfile("{}/tmplog.txt".format(cwd))
        print(cwd)
        print(path)
        return path
...
class Running(QObject):
    ...
    # defines a signal called running that takes no arguments
    running = pyqtSignal()

    # defines a signal called stopped that takes one int argument
    stopped = pyqtSignal(int, name='stopped')

    # defines a signal called status that has two overloads
    status = pyqtSignal([int], ['QString'])

    def emitting(self):
        self.running.connect(self.say_emmit)
        self.running.emit()
